			...
			return (
				<Row key={statistic.name+'/'+skill.name}>
					<Col sm={8}>
						<Row>
							<Col sm={4}>
						<div className="pretty p-icon p-smooth" style={{"fontSize": "1.5em", "marginRight":"10px"}}>
							<input type="checkbox" checked={this.state.fields[skill.field] !== 0} onChange={(e)=>this.handleCheckbox(skill.field)}/>
							<div className="state" >
								<div >
									<i className="icon">
										<FontAwesomeIcon icon={this.state.fields[skill.field] === 2 ? faCheckDouble : faCheck} size={"sm"} transform={"up-2.5"} color={"#3772FF"}/>
									</i>
								</div>
								<label></label>
							</div>
